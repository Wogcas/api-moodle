...
export class NotifyAssignmentTaskService {
    ...
    try {
      ...
      if (submissionTime > this.lastPollTime) {
        ...
        // En lugar de pasar 'submission' directamente, creamos el DTO
        const submissionDTO = this.mapSubmissionToDTO(assignment, submission);
        this.sendToRabbitMQ(submissionDTO);
      }
    } catch (error) {
  ...
  private mapSubmissionToDTO(assignment: any, submission: any): AssigmentSubmition {
    const timeModified = parseInt(submission.timemodified ?? '0', 10);
    const submissionTime = new Date(timeModified * 1000);
    const formattedTime = submissionTime.toLocaleString(); // Formato legible
  ...
  private async sendToRabbitMQ(submissionDTO: AssigmentSubmition) {
    try {
      // Pasamos el DTO a RabbitMQ
